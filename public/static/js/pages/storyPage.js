...
import { MAX_PAGES } from '../storyManager.js';
...
export class StoryPage {
    ...
    async handlePromptSubmit(userResponse) {
        ...
        // Check if we're at the last page before generating
        if (this.storyManager.storyPages.length >= MAX_PAGES - 1) {
            // We're about to generate the final page, so show conclusion instead
            console.log('Story complete! Showing conclusion screen. Pages:', this.storyManager.storyPages.length, 'Max:', MAX_PAGES);
        ...
        try {
            ...
            // Double-check after generation (shouldn't be needed but safety check)
